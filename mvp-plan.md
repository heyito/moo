...
> The smallest version of `got` that proves the one thing nobody else does:
...
> five-tool parallel-agent stack with one `got new`.
...
1. A machine's runtime state can be snapshotted and associated with a git
   HEAD SHA (`got save`).
2. `git checkout <old-sha>` + `got new <name>` deterministically restores
...
```
$ got new feat/billing                        # machine boots from base image
$ got run feat/billing -- ./setup-db.sh       # migration + seed applied
$ git commit -am "add billing migration"
$ got save feat/billing                       # snapshot tagged with HEAD SHA

$ got run feat/billing -- ./mutate-more.sh    # further state change
$ git commit -am "second migration"
$ got save feat/billing

$ git checkout HEAD^                          # rewind code one commit
$ got new feat/billing                        # boots snapshot for HEAD^
$ got run feat/billing -- ./verify-state.sh   # second migration is GONE

$ git checkout -                              # forward again
$ got new feat/billing                        # boots the later snapshot
$ got run feat/billing -- ./verify-state.sh   # second migration is BACK

$ got drop feat/billing                       # live machine gone
$ got new feat/billing                        # restored from snapshot; state intact
```
...
# After:
$ for name in agent-a agent-b agent-c; do got new $name from HEAD; done
$ got run agent-a -- npm run migrate          # each has its own DB,
$ got run agent-b -- npm run dev &            # own ports, own packages,
$ got run agent-c -- claude "refactor"        # own services — zero collisions
$ got save agent-b                            # keep the winner's runtime
$ git merge agent-b                           # promote via git
$ got drop agent-a agent-c                    # losers gone
```
...
- **`got doctor` and `got ls`** — read-only admin. `doctor` is required
...
- **Content-addressed snapshots** under `~/.got/snapshots/<content_hash>`
  with quiesce → `F_FULLFSYNC` → `clonefile` ordering and dedup
  (plan.md §5.3).
- **Guest exec + quiesce agent** — small static Rust binary in the golden
  image; runs commands for `got run` and `sync` + registered DB
  checkpoints for `got save`.
...
- **`got hook install/uninstall/status`.** The primitive is complete
  without hooks (plan.md §8.1 says so explicitly). The MVP demos use
  manual `got save` / `got new`; a shell alias in the docs covers
...
- **Full OCI/Dockerfile → rootfs build pipeline.** The MVP ships with a
  ...
  `got.toml` `base` field accepts these names. Arbitrary OCI references
...
The real crates (`got-cli`, `got-core`, `got-vmm`, `got-store`), built
on WP0's validated decisions.

- SQLite registry, both tables, provenance at `new`.
- Golden image cache under `~/.got/images/<hash>`; recipe hash =
...
**DoD:** `got new --detached from HEAD`, `got run`, `got drop` work
...
- `got save`: quiesce (guest `sync` + Postgres/SQLite/Redis checkpoint
...
- Snapshot-aware `got new`: existing handle looks up
  `(handle, current HEAD of shadowed ref)`; restore on hit, live
  overlay on miss.
- `got drop` preserves snapshots; `--snapshots` deletes them;
  `--force` kills unquiesceable machines.
- Restore-semantics messaging: when `got new` restores an older
...
- `got doctor`: entitlements, `libkrunfw`, APFS + `clonefile` on the
  store path, image cache, snapshot integrity.
- `got ls`: handles, lifecycle, port maps, snapshots per handle.
...
- Docs: README quickstart, the four verbs, restore semantics,
  the manual `git commit && got save` alias, the five-tool-stack
...
- Script and verify `git bisect run` with `got new` + `got run` in the
...
| `got new` on a warm 20 GB base | < 1 s |
| `got save` for a typical commit's state delta | < 1 s |
| `git checkout <old-sha>` + `got new` restore | deterministic, bit-exact overlay |
| Parallel machines (6+) with services and DBs | zero port/DB/env collisions, no helper scripts |
| `git bisect run` with got in the loop | works unattended on the demo repo |
...
- **Restore-vs-live-overlay confusion** — users lose unsaved runtime
  work when `got new` prefers a snapshot. *Response:* loud messaging in
...
First post-MVP release, in order of pull: `got hook install` (auto-follow
