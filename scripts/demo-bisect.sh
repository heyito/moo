...
#
# What it proves: `got save` at commit boundaries makes runtime state a
...
GOT="${GOT:-got}"
DEMO_DIR="${DEMO_DIR:-/tmp/got-bisect-demo}"
MACHINE="billing"
EXPECTED_TOTAL=6000   # cents; the sum a healthy database always reports

say() { printf '\n\033[1m== %s ==\033[0m\n' "$*"; }

sql() { "$GOT" run "$MACHINE" -- sqlite3 /srv/app.db "$1"; }
...
"$GOT" new "$MACHINE"
"$GOT" run "$MACHINE" -- 'apt-get update -q >/dev/null 2>&1 && apt-get install -y -q sqlite3 >/dev/null 2>&1 && mkdir -p /srv && echo tools ready'
...
migrate() { # migrate <n> <description> <sql>
    ...
    sql "$stmt" >/dev/null
    "$GOT" run "$MACHINE" -- sync >/dev/null
    git add -A
    git commit -q -m "migration $n: $desc"
    "$GOT" save "$MACHINE" >/dev/null
...
cat > /tmp/got-bisect-test.sh <<EOF
#!/bin/bash
# Boot this commit's saved runtime, then check the database invariant.
$GOT new $MACHINE >/dev/null 2>&1
actual=\$($GOT run $MACHINE -- sqlite3 /srv/app.db "SELECT SUM(price_cents) FROM products;")
echo "  probe \$(git rev-parse --short HEAD): total=\$actual"
[ "\$actual" = "$EXPECTED_TOTAL" ]
EOF
chmod +x /tmp/got-bisect-test.sh

git bisect start HEAD "$FIRST_GOOD" >/dev/null
git bisect run /tmp/got-bisect-test.sh > /tmp/got-bisect-out.txt 2>&1 || true
grep '  probe' /tmp/got-bisect-out.txt || true
...
rm -rf "$DEMO_DIR" /tmp/got-bisect-test.sh /tmp/got-bisect-out.txt
